...
% This script generates (panels for) figure 10, regarding the small sphere dynamics 
...
for i=1:numfiles
    ...
    %extract lid status - irrelevant in this data set
...
%% FIGURE 10 Display the stress dependence of the effective viscosity

masklid_len =  find(lengarr > 3); % select only the runs with sufficient number of datapoints 
...
densglass = mean(denspart(masklid)); % check if mean density is consistent with literature: yes

masklid = intersect(masklid,masklid_len);
%scatter(buoyarr(masklid),1./slopearr_nonlin(masklid,33), 90, buoyarr(masklid), '<','filled')
scatter(buoyarr(masklid),1./slopearr_lin(masklid), 90, sizearr(masklid), '<','filled')
%errorbar(buoyarr(masklid),1./slopearr_nonlin(masklid,33),minerr,maxerr,'.k')

pos = strfind(typearr,'steel');                 
masklid = find(~cellfun('isempty',pos)); 

denssteel = mean(denspart(masklid)); % check if mean density is consistent with literature: yes

masklid = intersect(masklid,masklid_len);
%scatter(buoyarr(masklid),1./slopearr_nonlin(masklid,33), 90, buoyarr(masklid), 's','filled')
scatter(buoyarr(masklid),1./slopearr_lin(masklid), 90, sizearr(masklid), 's','filled')
...
scatter(buoyarr(masklid),1./slopearr_lin(masklid), 90, sizearr(masklid), 'o','filled')
%errorbar(buoyarr(masklid),1./slopearr_nonlin(masklid,33),minerr,maxerr,'.k')
...
print(horzcat(('small-stressdep'), num2str(0,2)), '-dpdf', '-bestfit')
...
masknan = ~isnan(bestexparr);

%fit trend to exponent: goes down with size, consistent with sinking to
%rigid boundary (should be around 0.5 though, for 40mm)

f = fit(sizearr(masknan),bestexparr(masknan),'poly1');
plot(f,sizearr,bestexparr)
